...
public UserVo getUser(){
	...
	//实现登陆请求的转发
		...
		if(dao.canLogin(user)){ //登陆成功,显示主菜单
			...
			String sql="SELECT * FROM tuser WHERE user_account='"+user.getUserAccount()+"'";
	...
	//实现数据导出子菜单的转发
	public void exportSubMenuRequestProcess(int choice) throws RowsExceededException, WriteException, IOException, SQLException{
		SaleDao saleDao=new SaleDao();
		switch (choice){
		case 1:{
